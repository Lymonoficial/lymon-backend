...
import { R2StorageService, R2_STORAGE_SERVICE } from '@/infrastructure/storage/r2-storage.service';
...
export class GeneratePresignedUrlQueryHandler
  implements IQueryHandler<GeneratePresignedUrlQuery, GeneratePresignedUrlResult>
{
  constructor(
  ...
  async execute(query: GeneratePresignedUrlQuery): Promise<GeneratePresignedUrlResult> {
    const sanitizedName = query.fileName.replace(/[^a-zA-Z0-9._-]/g, '_');
    const key = `${query.tenantId}/${Date.now()}-${sanitizedName}`;

    const presignedUrl = await this.storageService.generatePresignedPutUrl(key, query.contentType);
