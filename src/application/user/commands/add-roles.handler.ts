...
import { PROPERTY_REPOSITORY, type PropertyRepository } from '@/domain/property/repositories/property.repository';
import { UNIT_REPOSITORY, type UnitRepository } from '@/domain/unit/repositories/unit.repository';
import { ROLE_REPOSITORY, type RoleRepository } from '@/domain/role/repositories/role.repository';
...
export class AddRolesHandler implements ICommandHandler<AddRolesCommand> {
  ...
  async execute(command: AddRolesCommand): Promise<void> {
    ...
    for (const a of command.roleAssignments) {
      const exists = existing.some((e) => e.roleId === a.roleId && JSON.stringify(e.scope) === JSON.stringify(a.scope));
  ...
  private async validateRoleAssignments(assignments: RoleAssignment[], tenantId: string) {
    ...
    for (const assignment of assignments) {
      ...
      if (!role) throw new BadRequestException(`Role '${assignment.roleId}' does not exist`);
      ...
      if (assignment.scope.type === 'PROPERTY') {
        if (!validPropertyIds) {
          ...
          validPropertyIds = new Set(properties.map((p) => p.getId()!.toString()));
        }
        const invalid = assignment.scope.resourceIds.filter((id: string) => !validPropertyIds!.has(id));
        if (invalid.length) throw new BadRequestException(`Property IDs not found: ${invalid.join(',')}`);
      }
      ...
      if (assignment.scope.type === 'UNIT') {
        ...
        const invalid = assignment.scope.resourceIds.filter((id: string) => !validUnitIds!.has(id));
        if (invalid.length) throw new BadRequestException(`Unit IDs not found: ${invalid.join(',')}`);
      }
    }
  }
}
