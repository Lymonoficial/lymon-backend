...
export default function loadReservationsList() {
    const baseUrl = getRequiredEnv('BASE_URL');
    const page = getEnvNumber('PAGE', 1);
    const limit = getEnvNumber('LIMIT', 20);
    const response = http.get(`${baseUrl}/reservations?page=${page}&limit=${limit}`, {
        headers: getRuntimeAuthHeaders(baseUrl),
    });
    check(response, {
        'reservations returns 200': () => response.status === 200,
    });
    sleep(thinkTime);
}
