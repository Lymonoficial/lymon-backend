...
export default function loadUnits() {
    ...
    const response = http.get(`${baseUrl}/units/${encodeURIComponent(propertyId)}?page=${page}&limit=${limit}`, {
        headers: getRuntimeAuthHeaders(baseUrl),
    });
    check(response, {
        'units returns 200': () => response.status === 200,
    });
    sleep(thinkTime);
}
