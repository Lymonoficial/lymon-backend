...
function getRuntimeAuthHeaders(baseUrl) {
  ...
  if (loginResponse.status < 200 || loginResponse.status >= 300) {
    throw new Error(
      `Auth login failed with status ${loginResponse.status}. Set TOKEN or valid AUTH_EMAIL/AUTH_PASSWORD.`,
    );
  }
  ...
  if (!token) {
    throw new Error(
      'Auth login succeeded but no accessToken was found in response.data.accessToken.',
    );
  }
  cachedToken = token;
...
export default function loadInventoryItems() {
  ...
  const response = http.get(
    `${baseUrl}/properties/${propertyId}/inventory/items?page=${page}&limit=${limit}`,
    {
