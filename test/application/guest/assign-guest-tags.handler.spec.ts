...
describe('AssignGuestTagsHandler', () => {
  ...
  describe('when all tags exist in the catalog', () => {
    it('assigns validated GuestTag objects to the guest and saves', async () => {
      ...
      const family = makeGuestTag({ name: 'family', id: '65f1a1a2b3c4d5e6f7000002' });
      ...
      const command = new AssignGuestTagsCommand(GUEST_ID, ['vip', 'family'], TENANT_ID);
      await handler.execute(command);

      expect(tagRepository.findByNames).toHaveBeenCalledWith(['vip', 'family'], TENANT_ID);
      expect(guestRepository.save).toHaveBeenCalledTimes(1);
      const savedGuest = guestRepository.save.mock.calls[0][0];
      expect(savedGuest.getTags().map((t) => t.getName())).toEqual(['vip', 'family']);
    });
  });
  ...
  describe('when tags are submitted in mixed case', () => {
    it('normalizes to lowercase before catalog lookup', async () => {
      ...
      const command = new AssignGuestTagsCommand(GUEST_ID, ['VIP', '  Vip  '], TENANT_ID);
      await handler.execute(command);

      expect(tagRepository.findByNames).toHaveBeenCalledWith(['vip'], TENANT_ID);
    });
  });
  ...
  describe('when duplicate tags are submitted', () => {
    it('deduplicates before catalog lookup', async () => {
      ...
      const command = new AssignGuestTagsCommand(GUEST_ID, ['vip', 'vip', 'VIP'], TENANT_ID);
      await handler.execute(command);
  ...
  describe('when all submitted tags are unknown', () => {
    it('throws BadRequestException', async () => {
      ...
      const command = new AssignGuestTagsCommand(GUEST_ID, ['ghost'], TENANT_ID);
  ...
  describe('when the guest does not exist', () => {
    it('throws NotFoundException', async () => {
      tagRepository.findByNames.mockResolvedValue([makeGuestTag({ name: 'vip' })]);
